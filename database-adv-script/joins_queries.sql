...
SELECT p.property_id, p.host_id, p.name, p.description, p.location, p.price_per_night, r.rating, r.comment from property as p LEFT JOIN reviews as r on r.property_id = p.id ORDER BY p.id
